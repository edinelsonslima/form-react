import { findMajority as findMajorityInCandidates } from './helpers';

// não usar caracteres especiais de regex na 'key' e.g. '.' '*' '+' '?' '^' '$' '(' ')' '[' ']' '{' '}' '|' '\'
const maskConfig: Record<string, Record<'clean' | 'match', RegExp>> = {
...
export function setMask(masks: string, input: string) {
...
export function clearMask(masks: string, input: string) {
  const mask = findSuitableMask(masks, input);
  if (!mask) return input;
